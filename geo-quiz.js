...
/* Music */
var musicGameElm = document.getElementById("musicGame");
musicGameElm.volume = 0.3;
var musicClickElm = document.getElementById("musicClick");
musicClickElm.volume = 1;
var musicAnswerCorrectElm = document.getElementById("musicAnswerCorrect");
musicAnswerCorrectElm.volume = 1;
var musicAnswerWrongElm = document.getElementById("musicAnswerWrong");
musicAnswerWrongElm.volume = 1;
var musicResultsElm = document.getElementById("musicResults");
musicResultsElm.volume = 0.7;
...
function timeCounter() {
    ...
    if (jokerWanted === 1) {
        ...
        timeLeft = 20; /* Extend Time to 20 Seconds */
        jokerWanted = 0;
    }
    ...
    if (answerSelected === 1) {
        clearTimeout(timeCounting); /* Stop Counter */
    ...
    if (timeLeft === 0) {
        if (answerSelected === 0) {
            ...
            addAnswerClass(); /* Display Correct/Wrong Answers */
            answerSelected = 2;
            sessionStorage.answerSelected = JSON.stringify(answerSelected);
            buttonStep02Elm.classList.add("buttonUnlocked");
            musicAnswerWrongElm.play();
            alert("Time's up! Close to show the correct answer.");
            clearTimeout(timeCounting); /* Stop Counter */
            return; /* Stop Function */
        }
    }
...
function gameStep01() {
    ...
    sectionHeaderElm.classList.add("sectionHeaderGame");
...
/* Step 2: Quiz */
let buttonStep02Elm;
let answerSelected = 0; /* 0 — not selected, 1 — selected (answered), 2 — selected (not answered) */
...
function gameStep02() {
    ...
    /* HTML Element: Button to Continue */
    buttonStep02Elm = document.createElement("button");
    ...
    if (firstQuestionGenerated === 0) {
        ...
        if (answerSelected === 1 || answerSelected === 2) {
            /* Style on the Button */
            buttonStep02Elm.classList.add("buttonUnlocked");
        ...
        setTimeout(() => {
            ...
            if (questionCounter !== 12) {
                ...
                if (answerSelected === 1 || answerSelected === 2) {
                    ...
                    alert("Select an answer before proceeding to results");
                    return;
                }
            }
        }, 200);
...
function gameStep03() {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        } else if (answersStorage[i].status === "wrong") {
            resultStatusElm.classList.add("resultValueWrong");
        } else {
            resultStatusElm.classList.add("resultValueNotAnswered");
        }
